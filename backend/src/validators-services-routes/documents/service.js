...
service.addDocument = async (req, res) => {
  try {
    ...
    if (!filePath) {
      return res
        .status(400)
        .json({ error: 'Missing required parameter - document this is error' })
    }
...
service.getDocumentBySubject = async (req, res) => {
  try {
    ...
    const documents = await Document.find({ subject: subjectId })
      .populate('addedBy')
      .lean()
...
service.deleteDocument = async (req, res) => {
  try {
    const { id } = req.params
    const userId = req.user._id
    const isPresent = await Document.findOne({
      _id: new ObjectId(id),
      addedBy: new ObjectId(userId),
    }).lean()
    if (!isPresent) {
    ...
    const deleteFileCludinary = await fileUploader.deleteDocument(
      isPresent.public_Id,
      isPresent.resource_type
    )
